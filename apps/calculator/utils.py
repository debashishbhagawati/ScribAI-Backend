...
def analyze_image(img : Image, dict_of_vars : dict):
    ...
    print("Raw Gemini response:", repr(response_text))
    ...
    try:
        ...
        print("Response text was:", repr(response_text))
        # Fallback: treat as plain English, try to fix missing spaces
        fixed_text = response_text
        # If the text is long and has no spaces, try to add spaces before capital letters
        if len(fixed_text) > 12 and " " not in fixed_text:
